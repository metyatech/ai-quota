...
- **CLI default exit behavior:** Keep default command execution successful (`exit code 0`) when quota/status results are reported, including partial provider failures such as `LOGIN_REQUIRED`; `--strict` continues to fail on provider fetch errors.

### Changed

- **Release publish guardrails:** Strengthened trusted publishing workflow verification and documented the canonical GitHub Release -> GitHub Actions -> npm trusted publishing path.

## [1.2.4] - 2026-03-18

### Fixed

- **Trusted publishing compatibility:** Run the publish workflow on Node.js 24 and update GitHub Actions to Node 24-compatible versions so npm trusted publishing can mint publish credentials successfully in CI.

## [1.2.3] - 2026-03-18

### Added

- **Trusted publishing workflow:** Publish releases to npm from GitHub Actions using npm trusted publishing (OIDC), so package publishing no longer depends on a long-lived npm token stored in the repository workflow configuration.
- **Dependabot automation:** Track weekly updates for both npm dependencies and GitHub Actions versions via `.github/dependabot.yml`.

### Changed

- **Release process:** Document the release flow around the new GitHub Actions publish workflow and GitHub Release trigger.

## [1.2.2] - 2026-03-18

### Fixed

- **Copilot CLI sign-in detection:** Resolve Copilot tokens using the official env var order (`COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN`) and reuse stored Copilot CLI OAuth tokens, including Windows Credential Manager entries, so `ai-quota` no longer reports `LOGIN_REQUIRED` when Copilot CLI is already signed in.
...
- **Claude Sonnet-only 7-day quota:** Display the `seven_day_sonnet` usage window separately from the all-models `seven_day` window, with "(all models)" / "(sonnet only)" suffixes when both are present.

## [1.0.1] - 2026-02-20

### Fixed

- **Codex:** Parse the ChatGPT `wham/usage` response shape that provides `rate_limit.primary_window/secondary_window` (in addition to the legacy `rate_limits.primary/secondary` shape).
